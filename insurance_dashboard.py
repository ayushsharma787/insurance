...
def _clean_df(df: pd.DataFrame) -> pd.DataFrame:
    """Apply all feature engineering to a raw dataframe."""
    df = df.copy()
...
def load_data() -> pd.DataFrame:
    """Load Insurance.csv from the same directory as this script."""
...
@st.cache_data
def load_uploaded(raw_bytes: bytes) -> pd.DataFrame:
    """Load a user-uploaded CSV from raw bytes."""
    import io
